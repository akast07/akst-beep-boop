...
function humanize(s) {
  const seq = mv.sequences.clone(s);
...
function concatenateSequences(seqs) {
  const seq = mv.sequences.clone(seqs[0]);
  let numSteps = seqs[0].totalQuantizedSteps;
  for (let i=1; i<seqs.length; i++) {
    const s = mv.sequences.clone(seqs[i]);
...
function generateProgressions(doneCallback) {
  ...
  generateInterpolations(0, temp, seqs => {
    ...
    progSeqs = concatSeqs.map(seq => {
      const mergedSeq = mv.sequences.mergeInstruments(seq);
      const progSeq = mv.sequences.unquantizeSequence(mergedSeq);
      progSeq.ticksPerQuarter = STEPS_PER_QUARTER;
      return progSeq;
    });
    
    const fullSeq = concatenateSequences(concatSeqs);
    const mergedFullSeq = mv.sequences.mergeInstruments(fullSeq);
...
function playProgression(chordIdx) {
  ...
  const unquantizedSeq = mv.sequences.unquantizeSequence(chordSeqs[idx][chordIdx]);
...
function saveSequence() {
  const idx = alphaSlider.value;
  const midi = mv.sequenceProtoToMidi(progSeqs[idx]);
...
function chordChanged() {
  const isGood = (chord) => {
    ...
    try {
      mv.chords.ChordSymbols.pitches(chord);
      return true;
    } catch(e) {
      return false;
    }
...
function initPlayerAndEffects() {
  ...
  globalLimiter.connect(mv.Player.tone.Master);
  ...
  const player = new mv.SoundFontPlayer(
